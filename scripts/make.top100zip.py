...
if __name__ == '__main__':
    args = optionparser.get_options(
        [('o', 'output', 'Output directory', True), ('f', 'format', 'Format to include', True),
         ('t', 'temp', 'Temporary directory', False), ])
    ...
    try:
        if not isinstance(getattr(Image.Archive, fmt), ResourceManager):
            raise TypeError("Specified format is not a valid resource format.")
